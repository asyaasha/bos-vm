...
class VmStack {
 ...
 renderElement(code) {
  ...
  (code.type === "JSXFragment"
   ...
   if (attribute.type === "JSXAttribute") {
    ...
    attributes[name] =
     attribute.value === null ? true : this.executeExpression(attribute.value);
 ...
 callFunction(keyword, callee, args, optional, isNew) {
  ...
  console.log("debugg");
  console.log(keyword, callee, args);
  throw new Error(
  ...
  if (code.type === "Identifier" || code.type === "JSXIdentifier") {
   ...
   if (
    ...
    if (keyword in Keywords) {
     if (!options?.callee) {
      throw new Error(
       "Cannot dereference keyword '" + keyword + "' in non-call expression"
      );
     }
     return {
 ...
 executeExpressionInternal(code) {
  ...
  if (type === "AssignmentExpression") {
   ...
   const canCache =
    code.quasi.expressions.length === 0 && code.tag.type !== "CallExpression";
 ...
 createFunction(params, body, isExpression) {
  ...
  return (...args) => {
   ...
   const isTrusted = !!(
    args?.[0]?.nativeEvent instanceof Event && args?.[0]?.nativeEvent.isTrusted
   );
...
export default class VM {
 constructor(options) {
  ...
  this.stProvider = stProviderContext?.arProvider
   ? stProviderContext?.arProvider
   : null;
 ...
 cachedEthersCall(callee, args, subscribe) {
  return this.cachedPromise(
   (invalidate) =>
    this.cache.cachedEthersCall(this.ethersProvider, callee, args, invalidate),
   subscribe
  );
 }
 ...
 cachedStCall(callee, args, subscribe) {
  return this.cachedPromise(
   (invalidate) =>
    this.cache.cachedStCall(this.stProvider, callee, args, invalidate),
   subscribe
  );
 }
