...
import { Provider, SequencerProvider, constants, RpcProvider, Account, json, Contract } from 'starknet';
import { Web3ConnectButton } from '../components/ethers';
import { StConnectButton } from '../components/starknet';
...
const frozenSt = Object.freeze({
  stProvider: deepFreeze(Provider),
  stAccount: deepFreeze(Account),
  stContract: deepFreeze(Contract),
  stJson: deepFreeze(json),
  stConstants: deepFreeze(constants),
  stConnect: deepFreeze(connect),
});
...
class VmStack {
  ...
  renderElement(code) {
    ...
    } else if (element === 'StConnect') {
      return <StConnectButton {...attributes} />;
    } else if (RadixComp) {
  ...
  callFunction(keyword, callee, args, optional, isNew) {
    ...
    if (keywordType === true || keywordType === undefined) {
      if ((keyword === 'Social' && callee === 'getr') || callee === 'socialGetr') {
        ...
        return this.vm.cachedNearBlock(blockId, maybeSubscribe(subscribe, blockId));
      ...
      } else if (keyword === 'Stark') {
        console.log('CALL STARK');
        console.log(callee);
        if (callee === 'provider') {
          return this.vm.stProvider;
        }
        return this.vm.cachedStCall(callee, args);
...
export default class VM {
  constructor(options) {
    ...
    this.stProviderContext = stProviderContext;
    this.stProvider = stProviderContext?.arProvider ? stProviderContext?.arProvider : null;
  ...
  cachedSocialGet(keys, recursive, blockId, options) {
    ...
    return this.cachedPromise(
      (invalidate) => this.cache.socialGet(this.near, keys, recursive, blockId, options, invalidate),
      options?.subscribe,
    );
  }
  ...
  cachedEthersCall(callee, args, subscribe) {
    return this.cachedPromise(
      (invalidate) => this.cache.cachedEthersCall(this.ethersProvider, callee, args, invalidate),
      subscribe,
    );
  }

  cachedStCall(callee, args, subscribe) {
    return this.cachedPromise(
      (invalidate) => this.cache.cachedStCall(this.stProvider, callee, args, invalidate),
      subscribe,
    );
  }
  ...
  cachedNearView(contractName, methodName, args, blockId, subscribe) {
    return this.cachedPromise(
      (invalidate) => this.cache.cachedViewCall(this.near, contractName, methodName, args, blockId, invalidate),
      subscribe,
    );
  }

  cachedNearBlock(blockId, subscribe) {
    return this.cachedPromise((invalidate) => this.cache.cachedBlock(this.near, blockId, invalidate), subscribe);
  }
  ...
  cachedFetch(url, options) {
    return this.cachedPromise((invalidate) => this.cache.cachedFetch(url, options, invalidate), options?.subscribe);
  }

  cachedIndex(action, key, options) {
    return this.cachedPromise(
      (invalidate) => this.cache.socialIndex(this.near, action, key, options, invalidate),
      options?.subscribe,
    );
  }
  ...
  getVmInstance(code, src) {
    ...
    const vm = new VM({
      ...
      stProviderContext: this.stProviderContext,
      isModule: true,
    });
